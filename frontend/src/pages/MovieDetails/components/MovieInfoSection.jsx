...
const movie = movieDetailResponse.data[0];
const formattedRelDate = new Date(movie.release_date).toLocaleDateString("en-GB");

const durationDetail1 = movie.duration?.replace("h", " hours") || "";
const duration = durationDetail1.replace("m", " minutes");

const numericRating = Number(movie.rating);
const safeRating = isNaN(numericRating) ? "0.0" : numericRating.toFixed(1);
