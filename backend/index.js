...
app.use(
  cors({
    origin: ["https://quick-show-tlzv.vercel.app","http://localhost:5173"],
...
SELECT
  ...
  PA.id AS payment_id,
  GROUP_CONCAT(DISTINCT T.id SEPARATOR ', ') AS ticket_ids,
  GROUP_CONCAT(DISTINCT ST.name SEPARATOR ', ') AS seat_numbers,
  MAX(TH.name) AS theatre_name,
  MAX(H.name) AS hall_name,
  MAX(M.name) AS movie_name,
  MAX(T.movie_id) AS movie_id,
  MAX(M.image_path) AS movie_image,
  MAX(S.movie_start_time) AS movie_start_time,
  MAX(S.show_type) AS show_type,
  MAX(S.showtime_date) AS showtime_date,
  MAX(PA.amount) AS ticket_price,
